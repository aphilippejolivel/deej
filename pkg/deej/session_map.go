...
func (m *sessionMap) setupOnSliderMove() {
	...
	switchEventsChannel := m.deej.serial.SubscribeToSwitchToggleEvent()
	...
	go func() {
		for {
			...
			case event := <-switchEventsChannel:
				m.handleSwitchToggleEvent(event)
			}
		}
	}()
}
...
func (m *sessionMap) handleSwitchToggleEvent(event SwitchToggleEvent) {
	...
	// get the targets mapped to this slider from the config
	targets, ok := m.deej.config.SliderMapping.get(event.SwitchID)
	...
	targetFound := false
	adjustmentFailed := false
	mic_target := targets[idx:idx]
	
	// for each possible target for this slider...
	for _, target := range mic_target {

		m.logger.Warnw("Found MIC")
		// resolve the target name by cleaning it up and applying any special transformations.
		// depending on the transformation applied, this can result in more than one target name
		resolvedTargets := m.resolveTarget(target)
		...
		for _, resolvedTarget := range resolvedTargets {
			...
			targetFound = true
			newBool := event.Value != 0
			m.logger.Warnw("bool is:","bool", newBool)
			// iterate all matching sessions and adjust the volume of each one
			for _, session := range sessions {
				if session.GetMute() != newBool {
					if err := session.SetMute(newBool); err != nil {
						m.logger.Warnw("Failed to set target session volume", "error", err)
						adjustmentFailed = true
					}
				}
